...
```
*Flag: `--db` (`sqlite`/`mysql`/`pgsql`/`none`), `--admin-pass`, `--admin-url`, `--spa` (`yes`/`no`), `--scaffold` (`yes`/`minimal`/`no`), `--htaccess` (`yes`/`no`).*
...
*   **`data-fst-scroll="instant|smooth|false"`**
    Mengontrol perilaku scroll setelah halaman selesai dimuat via SPA.
    - `instant` (Default): Menggulung target (body atau kontainer) ke atas secara instan (seketika).
    - `smooth`: Menggulung target ke atas dengan efek transisi halus.
    - `false`: Mematikan reset scroll (posisi scroll layar/kontainer tetap berada di tempatnya saat ini). Sangat berguna untuk pagination dinamis, pencarian instan, atau tab dinamis.
    ```html
    <!-- Pindah halaman dan scroll ke atas secara halus -->
    <a href="/profil" data-fst-scroll="smooth">Lihat Profil</a>
    
    <!-- Ambil rute pencarian tanpa memindahkan fokus scroll pengguna -->
    <form action="/cari" method="GET" data-fst-target=".hasil" data-fst-scroll="false">
    ```

### Reset & Restorasi Scroll Otomatis
1. **Navigasi Rute Baru**: Secara default, memuat rute baru akan mereset scroll ke atas secara instan (`instant`) agar pengguna langsung fokus ke header halaman baru.
2. **Back & Forward (Scroll Restoration)**: FullStuck secara cerdas menyimpan koordinat scroll halaman sesaat sebelum berpindah halaman. Ketika pengguna mengklik tombol Back/Forward di browser, koordinat scroll terakhir mereka pada halaman tersebut akan dipulihkan secara otomatis.
3. **Anchor Link**: Klik link jangkar lokal (seperti `<a href="#fitur">`) akan diabaikan oleh SPA agent agar browser melakukan scroll bawaan. Jika rute jangkar berada di halaman lain (seperti `<a href="/tentang#tim">`), SPA agent akan memuat halaman `/tentang` terlebih dahulu lalu menggulir ke elemen `#tim` secara mulus (`smooth`).
...
*   **`fst:loading`**: Dikirim ketika fetch (navigasi / submit form) baru saja dimulai. Membawa informasi detail rute yang dipanggil di dalam properti `e.detail` (`url`, `targetSelector`, `triggerElement`). **Event ini bersifat `cancelable`**, sehingga Anda dapat memanggil `e.preventDefault()` untuk membatalkan proses navigasi SPA (misalnya, untuk mendeteksi form kotor sebelum pindah halaman).
*   **`fst:unload`**: Dikirim setelah data berhasil didapat dari server, tepat sebelum HTML disuntikkan ke DOM. Berguna untuk menghapus memory leak / mematikan plugin JS lama (misal: Select2 destroy).
*   **`fst:load`**: Dikirim setelah HTML baru disuntikkan ke DOM dan seluruh script selesai dijalankan ulang. Berguna untuk inisialisasi ulang plugin JS.
...
// Contoh membatalkan navigasi jika form kotor
document.addEventListener('fst:loading', (e) => {
    if (window.isFormDirty && !confirm('Perubahan belum disimpan. Yakin ingin keluar?')) {
        e.preventDefault(); // Membatalkan navigasi SPA
    }
});
...
### Programmatic Routing API (Global `fst.go`)

Selain menggunakan atribut deklaratif pada tag HTML, FullStuck juga menyediakan API JavaScript global untuk mengontrol navigasi SPA secara programmatis dari dalam file JavaScript eksternal atau event listener.

#### Objek Global `window.fst`

*   **`fst.go(url, options)`**
    Memicu pemuatan navigasi SPA ke URL tertentu secara terprogram.
    *   `url` (string): URL tujuan.
    *   `options` (object, opsional):
        *   `target` (string): Selector CSS untuk elemen kontainer target (default: sesuai konfigurasi `default_target` di `fullstuck.json`).
        *   `history` (boolean): Set ke `false` jika tidak ingin mengubah URL di address bar (default: `true`).
        *   `scroll` (string|boolean): Perilaku scroll setelah halaman dimuat (`'instant'`, `'smooth'`, atau `false` untuk menonaktifkan scroll reset).
        *   `indicator` (string): Class CSS loading indicator untuk elemen target.

```javascript
// Navigasi dasar dari HTML (Inline event)
<button onclick="fst.go('/dashboard')">Dashboard</button>

// Navigasi kustom dari file JS eksternal
fst.go('/profile', {
    target: '#main-content',
    history: false,
    scroll: 'smooth'
