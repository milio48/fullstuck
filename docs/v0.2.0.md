...
> "Unduh `https://raw.githubusercontent.com/milio48/fullstuck/main/fullstuck.php` lalu jalankan `php fullstuck.php init --db=sqlite --admin-pass=admin --scaffold=yes`. Pelajari `v0.2.0.md`. Jalankan `php -S localhost:8000 fullstuck.php` dan berikan ringkasan instalasi."
...
```
*Flag: `--db` (`sqlite`/`mysql`/`pgsql`/`none`), `--admin-pass`, `--admin-url`, `--spa` (`yes`/`no`), `--scaffold` (`yes`/`no`).*
