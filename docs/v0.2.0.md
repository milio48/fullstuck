...
3. **Setup Wizard:** Buka `http://localhost:8000` di browser Anda. Framework akan otomatis mendeteksi instalasi baru dan menampilkan **GUI Setup Wizard**.
   ...
   * Tersedia opsi untuk men-download file panduan ini (`v0.2.0.md`) untuk referensi asisten AI Anda.
...
```
Perintah ini akan langsung mem-bypass web installer, men-generate `fullstuck.json`, mengunduh dokumentasi `v0.2.0.md`, dan membuat file *starter* jika `--scaffold=yes` disertakan. Jika `fullstuck.json` sudah ada, perintah ini otomatis ditolak.
...
<a name="dom-templating"></a>
## 9. 🚀 Fitur Baru v0.2.0: Procedural DOM Templating (`fst_template`)

FullStuck.php v0.2.0 memperkenalkan engine templating revolusioner berbasis **DOM Manipulation**. Tidak seperti Blade atau Twig yang mengharuskan Anda menyisipkan tag khusus (seperti `{{ }}` atau `@if`) ke dalam file HTML, pendekatan ini mengizinkan Anda menjaga file HTML 100% statis dan bersih. Seluruh "penjahitan" data dilakukan di level PHP menggunakan sintaks deklaratif (Mirip konsep Enlive).

**Keunggulan Utama:**
1. **Frontend-Agnostic:** File `.html` Anda dapat diedit langsung oleh Web Designer atau di-preview di browser tanpa error sintaks PHP/Template.
2. **Zero XSS by Default:** Hampir seluruh injeksi teks dilakukan via properti text content, sehingga secara otomatis kebal terhadap serangan XSS.
3. **Deklaratif & Sentralisasi Logic:** Seluruh logika bisnis (IF, Loop) tetap murni di wilayah Controller (PHP).
4. **Caching Agresif:** Framework akan melakukan _compile_ DOM secara internal untuk memastikan kecepatan yang setara dengan Native PHP include.

### Contoh Kasus (Blog List)

**1. File HTML Statis (`views/blog-list.html`)**
```html
<!DOCTYPE html>
<html lang="id">
<head>
...
### 📐 Ruleset Syntax (DSL) API

API ini menggunakan konsep **Declarative Nested Scope**. Anda memetakan elemen (CSS Selector) sebagai key array, dan daftar instruksinya di dalam parameter sub-array.

#### 1. Text Injection (Shorthand)
Digunakan untuk menimpa konten teks. **Wajib** berupa referensi nama variabel dalam format string. Semua teks otomatis dilindungi via XSS Escape.
```php
"h1.title" => '$titleVar'
```

#### 2. Attribute Directive `[attr]`
Memanipulasi atribut HTML.
```php
"img.thumbnail" => [
    "[src]" => '$imageSource',
...
Menduplikasi elemen. Compiler otomatis mencuplik elemen pertama sebagai "cetakan" dan menghapus sibling dummy lainnya.
```php
"ul.nav > li" => [
...
#### 4. Logic Directive `@if`
Menyembunyikan / Menampilkan elemen berdasarkan kondisi.
```php
"div.banner-promo" => [
    "@if" => '$isPromoActive'
]
...
#### 5. Logic Directive `@html`
Merender Raw HTML langsung dari string tanpa escaping XSS. Cocok untuk input *WYSIWYG Editor*. Peringatan: ini akan menimpa (replace) node anak dari elemen parent.
```php
"div.content" => [
...
#### 6. Logic Directive `@append` & `@prepend`
Injeksi *Raw HTML* secara non-destruktif tanpa mengganggu node anak yang sudah ada. Sangat berguna untuk menyuntikkan `<style>` dan `<script>`.
```php
"head" => [
    // Menyisipkan tag <style> tepat sebelum tag penutup </head>
    "@append" => '"<style>.dark-mode { background: #000; }</style>"'
],
"body" => [
    // Menyisipkan HTML setelah tag pembuka <body>
    "@prepend" => '"<div class=\"alert\">Anda login!</div>"'
]
```

#### 7. Menghapus Atribut (`@remove`)
Menghapus atribut bawaan dari template statis.
```php
"img.thumbnail" => [
    "[style]" => "@remove",       // Hapus inline style dummy
    "[src]" => '$realImageUrl'
]
```
