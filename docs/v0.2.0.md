...
> **Note untuk `fst_template`**: Variabel dari `fst_view_share` tidak di-inject otomatis ke dalam procedural templating. Anda harus meneruskannya secara eksplisit, misalnya `$data['site_name'] = fst_app('shared_view_data')['site_name'] ?? '';` sebelum memanggil `fst_template()`.
...
*   `fst_redirect($url, $code = 302, $allow_external = false)`: Redirect. **Return**: `void` (Otomatis exit).
*   `fst_abort($code, $message)`: Hentikan dengan error code HTTP (misal: 404, 500). **Return**: `void` (Otomatis exit).
*   `fst_serve_static_file($path)`: Menyajikan file aset dengan cache headers. **Return**: `bool`.
...
*   `fst_db($mode, $sql, $params, $connection)`: Raw Query PDO manual. Mode: `'ROW'`, `'ALL'`, `'SCALAR'` (atau `'ONE'`), dan `'EXEC'`.
    **Return**: `array` (ALL/ROW), `mixed` (SCALAR), atau `bool` (EXEC).
*   `fst_db_select($table, $cond, $opts)`: Mengambil data dari tabel. Opsi didukung: `select`, `limit`, `offset`, `order_by`, `connection`.
...
*   `fst_db_quote_ident($ident, $connection)`: Keamanan penamaan tabel/kolom lintas-DB. **Return**: `string`.
