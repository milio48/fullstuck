...
1. **Analisis Konteks**: Baca `docs/SYSTEM_MAP.md` dan pahami modul mana di dalam `src/` yang paling tepat untuk diubah.
...
- **`docs/CHANGELOG.md`**: Rekam setiap penambahan fitur, perubahan penting, atau fungsi yang dihapus di bawah section `[Unreleased]` agar histori versi terlihat jelas.
...
- **`docs/DOCUMENTATION.md`**: Apabila Anda membuat fungsi pembantu (*helper*) baru (misal: `fst_sesuatu()`), Anda WAJIB menambahkan deskripsi dan cara panggilannya di file ini.
- **`docs/ARCHITECTURE.md` atau `SYSTEM_MAP.md`**: Apabila Anda menambah file sistem baru atau merubah arsitektur *flow*.
