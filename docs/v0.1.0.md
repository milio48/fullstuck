...
4. [✨ Keajaiban SPA (Single Page Application)](#spa)
5. [🛠️ Studi Kasus: Membuat Mini CMS](#mini-cms)
6. [📚 API Reference (Kamus Fungsi)](#api-reference)
7. [🤖 Strict Rules for AI](#ai-rules)
...
- **`environment`**: Set ke `"development"` untuk melihat *error trace* secara detail di layar, atau `"production"` untuk menyembunyikan error sensitif dan memunculkan custom error 500.
- **`admin`**: Mengatur URL akses halaman *dashboard admin* serta menyimpan sandi dalam bentuk *hash bcrypt*.
- **`database`**: Anda dapat menyimpan banyak konfigurasi (mysql, sqlite, pgsql) secara bersamaan, dan cukup mengganti field `"driver"` untuk berpindah tipe database dengan mulus.
  - Set `"driver": "none"` jika aplikasi Anda tidak membutuhkan koneksi database. Fungsi `fst_db_*` akan menghasilkan error jika dipanggil dalam mode ini.
- **`routing`**: 
...
- **`spa`**: Mengontrol opsi tingkat lanjut mode Single Page Application, termasuk HTTP header *custom* untuk interaksi Ajax, *class indicator* saat proses `fetch` berjalan, dan target spesifik dari fragment SPA.
...
FullStuck otomatis menyiapkan koneksi PDO. Jangan gunakan fungsi PDO *native* secara manual, gunakan Query Builder bawaan agar kode ringkas dan *cross-database* (MySQL, PostgreSQL, SQLite).
...
// Update (Set status='inactive' WHERE id=5)
...
<a name="spa"></a>
## 4. ✨ Keajaiban SPA (Single Page Application)
...
<a name="mini-cms"></a>
## 5. 🛠️ Studi Kasus: Membuat Mini CMS
...
### Database
*   `fst_db($mode, $sql, $params)`: Raw Query PDO manual.
*   `fst_db_select($table, $cond, $opts)`: Mengambil array dari tabel.
*   `fst_db_insert($table, $data)`: Menambahkan baris data.
*   `fst_db_update($table, $data, $cond)`: Mengubah data.
*   `fst_db_delete($table, $cond)`: Menghapus data.
*   `fst_db_quote_ident($ident)`: Keamanan penamaan tabel/kolom lintas-DB.
...
*   `fst_validate($data, $rules)`: Engine validasi *array* input. Mendukung `required`, `email`, `numeric`, `min`, `max`, `min_value`, `max_value`.
