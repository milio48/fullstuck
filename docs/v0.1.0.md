...
*   `e($str)`: Anti-XSS (HTML Escape). Wajib saat *echo* variabel ke layar.
...
*   `fst_validate($data, $rules)`: Engine validasi *array* input.
...
*   `fst_app($key, $value)`: Akses ke memori internal (kontainer) framework.
