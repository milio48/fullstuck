...
- **XSS Escape Helper (`fst_escape` / `e`)**: Menambahkan fungsi `fst_escape($str)` dan alias pendeknya `e($str)` sebagai wrapper `htmlspecialchars()` dengan `ENT_QUOTES` dan `UTF-8`. Mempermudah *output escaping* di View untuk mencegah serangan XSS.
...
- **Fix Dynamic Routing `pages_dir`**: Fungsi `_fst_match_dynamic_routes()` kini menghormati konfigurasi `dynamic_config.pages_dir`. Sebelumnya, framework selalu mencari file di root project meskipun `pages_dir` sudah dikonfigurasi (contoh: `"pages_dir": "pages"`).
